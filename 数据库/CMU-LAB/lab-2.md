...
说回项目，可拓展哈希作为一种哈希表的实现，那就要实现一些统一的功能和结构。从结构来说，可拓展哈希有三层抽象，第一层是功能接口，第二层是存储中间层，第三层是存储层。三层之间为单向调用关系，负责的功能各不相同：

* 功能接口：增删查改，核心就是将key映射到中间层的索引上
* 存储中间层：存有哈希表的元信息，将key映射到对应的桶上
* 存储层：通过key在保存的键值对数组里面增删查改
