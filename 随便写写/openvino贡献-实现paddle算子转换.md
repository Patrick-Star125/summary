...
为openvino的paddle端实现算子映射不仅要实现算子逻辑，还要实现配套的**单元测试**，它们随算子的逻辑复杂程度不同而各不相同，我将我所实现的ceil、sum、group_norm这三个算子贴在下面，希望能够给你的开发提供一些参考。
...
这两个问题如果在正常的c++开发中很简单就能发现，但是在openvino的开发模式中，运行时调试比较困难，并且方法之间调用关系不太清晰，IDE不能帮到我们太多，小问题也会造成难以排查的错误，这反映出三点：
