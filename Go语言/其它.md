...
上述的`jsonrpc`可以满足跨语言的数据编解码的要求，但是也存在一些缺点，比如不支持http传输，数据编解码性能不高等。于是呢，一些第三方rpc库都选择采用`protobuf`进行数据编解码，并提供一些服务注册代码自动生成功能。

首先，需要安装`protobuf`及`protoc`可执行命令，然后，我们编写一个proto文件，定义要实现的RPC方法及其相关参数。
